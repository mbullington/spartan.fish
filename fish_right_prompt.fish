...
function __spartan_right_prompt_time
  ...
  __spartan_segment_right: grey blue (inline: (bold: " $prettydate "))
...
function fish_right_prompt
  ...
  if [ $last_status != 0 ]
    __spartan_segment_right: red grey (inline: (bold: " $last_status "))
  end

  if not test -z $fish_private_mode
    __spartan_segment_right: white grey (inline: " incognito ")
  end
  ...
  # command has to be running for more than 5 seconds
    ...
    __spartan_segment_right: green grey (inline: (bold: " "$prettyduration" "))
  end
